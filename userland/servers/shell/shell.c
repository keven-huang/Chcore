...
int do_complement(char *buf, char *complement, int complement_time)
{
        ...
        FILE *fp = fopen("/", "r");
        if (fp == NULL) {
                chcore_bug("fopen wrong");
                return -1;
        }
        ret = getdents(fp->fd, scan_buf, BUFLEN);
        ...
        do {
                struct ipc_msg *ipc_msg =
                        ipc_create_msg(fs_ipc_struct,
                                       sizeof(struct fs_request) + cnt + 2,
                                       0);
                chcore_assert(ipc_msg);
...
void print_file_content(char *path)
{
        ...
        int ret;
        ret = read_whole_file(fp->fd, buf);
        if(ret < 0)
                chcore_bug("cannot read file\n");
        printf("%s", buf);
        ret = fclose(fp);
        if (ret < 0)
                chcore_bug("cannot read file\n");
...
void fs_scan(char *path)
{
        ...
        if (fp == NULL)
                return;
        int ret = getdents(fp->fd, scan_buf, BUFLEN);
