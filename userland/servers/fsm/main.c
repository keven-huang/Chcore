                               ...
                               struct mount_point_info_node *mount_point_info)
{
        ...
        for_each_in_list (private_iter,
                          ...
                          node,
                          &fsm_mount_info_mapping) {
                if (private_iter->client_badge == client_badge) {
                        private_iter->mount_point_info[fd] = mount_point_info;
                        return;
                }
        }
        struct client_fd_info_node *n = (struct client_fd_info_node *)malloc(
                sizeof(struct client_fd_info_node));
...
struct mount_point_info_node *fsm_get_mount_info_withfd(u64 client_badge,
                                                        int fd)
{
        struct client_fd_info_node *n;
        for_each_in_list (
                n, struct client_fd_info_node, node, &fsm_mount_info_mapping)
...
void fsm_server_dispatch(struct ipc_msg *ipc_msg, u64 client_badge)
{
        ...
        switch (fr->req) {
        case FS_REQ_MOUNT:
                ret = fsm_mount_fs(fr->mount.fs_path,
                                   fr->mount.mount_path); // path=(device_name),
                                                          // path2=(mount_point)
                break;
        case FS_REQ_UMOUNT:
        ...
        case FS_REQ_GET_FS_CAP:
                mpinfo = get_mount_point(fr->getfscap.pathname,
                                         strlen(fr->getfscap.pathname));
        ...
        case FS_REQ_OPEN: {
                ...
                struct ipc_msg *ipc_msg2 = ipc_create_msg(
                        mpinfo->_fs_ipc_struct, sizeof(struct fs_request), 0);
                struct fs_request *fr2 =
                        (struct fs_request *)ipc_get_msg_data(ipc_msg2);
                fr2->req = FS_REQ_OPEN;
                fr2->open.new_fd = fr->open.new_fd;
                fr2->open.flags = fr->open.flags;
                fr2->open.mode = fr->open.mode;
                fr2->open.fid = fr->open.fid;
                strcpy(fr2->open.pathname, fr->open.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg2);
                ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg2);
                break;
        }
        case FS_REQ_CREAT: {
                ...
                struct fs_request *fr2 =
                        (struct fs_request *)ipc_get_msg_data(ipc_msg2);
                fr2->req = FS_REQ_CREAT;
                fr2->creat.mode = fr->creat.mode;
                strcpy(fr2->creat.pathname, fr->creat.pathname);
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg2);
                ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg2);
                break;
        }
        case FS_REQ_WRITE: {
                ...
                struct ipc_msg *ipc_msg2 = ipc_create_msg(
                        mpinfo->_fs_ipc_struct,
                        sizeof(struct fs_request) + fr->write.count,
                        0);
                struct fs_request *fr2 =
                        (struct fs_request *)ipc_get_msg_data(ipc_msg2);
                fr2->req = FS_REQ_WRITE;
                fr2->write.count = fr->write.count;
                fr2->write.fd = fr->write.fd;
                fr2->write.write_buff_begin = fr->write.write_buff_begin;
                memcpy((void *)fr2 + sizeof(struct fs_request),
        ...
        case FS_REQ_READ: {
                ...
                struct ipc_msg *ipc_msg2 = ipc_create_msg(
                        mpinfo->_fs_ipc_struct,
                        sizeof(struct fs_request) + fr->read.count,
                        0);
                struct fs_request *fr2 =
                        (struct fs_request *)ipc_get_msg_data(ipc_msg2);
                fr2->req = FS_REQ_READ;
				fr2->read.fd = fr->read.fd;
                fr2->read.count = fr->read.count;
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg2);
                memcpy(ipc_get_msg_data(ipc_msg),
                       ipc_get_msg_data(ipc_msg2),
                       ret);
                ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg2);
                break;
        }
        case FS_REQ_CLOSE: {
                ...
                struct fs_request *fr2 =
                        (struct fs_request *)ipc_get_msg_data(ipc_msg2);
                fr2->req = FS_REQ_CLOSE;
                fr2->close.fd = fr->close.fd;
                ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg2);
                ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg2);
                break;
        }
        case FS_REQ_GETDENTS64: {
                mpinfo = fsm_get_mount_info_withfd(client_badge,
                                                   fr->getdents64.fd);
                struct ipc_msg *ipc_msg2 =
                        ipc_create_msg(mpinfo->_fs_ipc_struct, 512, 0);
				struct fs_request *fr2 = (struct fs_request *)ipc_get_msg_data(ipc_msg2);
				fr2->req = FS_REQ_GETDENTS64;
				fr2->getdents64.fd = fr->getdents64.fd;
				fr2->getdents64.count = fr->getdents64.count;
				ret = ipc_call(mpinfo->_fs_ipc_struct, ipc_msg2);
				memcpy(ipc_get_msg_data(ipc_msg), ipc_get_msg_data(ipc_msg2), ret);
				ipc_destroy_msg(mpinfo->_fs_ipc_struct, ipc_msg2);
				break;
        }
                /* LAB 5 TODO END */
