...
int cap_group_init(struct cap_group *cap_group, unsigned int size, u64 pid)
{
        ...
        /* LAB 3 TODO BEGIN */
        int r;
        r = slot_table_init(slot_table, size);
        if (r < 0)
                return r;
        cap_group->pid = pid;
...
int sys_create_cap_group(u64 pid, u64 cap_group_name, u64 name_len, u64 pcid)
{
        ...
        new_cap_group = obj_alloc(CAP_GROUP_OBJ_ID, sizeof(struct cap_group));
        /* LAB 3 TODO END */
        ...
        /* 2st cap is vmspace */
        /* LAB 3 TODO BEGIN */
        vmspace = obj_alloc(VMSPACE_OBJ_ID, sizeof(struct vmspace));
        /* LAB 3 TODO END */
        if (!vmspace) {
                r = -ENOMEM;
